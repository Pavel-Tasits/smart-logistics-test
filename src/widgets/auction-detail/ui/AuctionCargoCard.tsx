...
export function AuctionCargoCard({
  ...
  const hasTemperatureRange = cargo.tempFrom != null || cargo.tempTo != null;
  ...
  return (
    <Card withBorder radius="md" padding="md" h="100%">
      <Stack component="dl" gap="sm">
        ...
        <DetailField label="Наименование">{cargo.name}</DetailField>

        <Group gap="lg">
          <DetailField label="Кузов">{cargo.bodyType ?? '—'}</DetailField>

          <DetailField label="Расстояние">
            {cargo.distanceKm != null ? `${cargo.distanceKm} км` : '—'}
          </DetailField>

          <DetailField label="Кол-во ТС">{cargo.truckCount ?? '—'}</DetailField>
        </Group>
        ...
        {hasTemperatureRange && (
          <DetailField label="Температурный режим">
            {formatTemperatureRange(cargo.tempFrom, cargo.tempTo)}
          </DetailField>
        )}
