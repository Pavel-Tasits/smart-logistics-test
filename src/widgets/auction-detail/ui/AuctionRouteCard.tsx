import { Alert, Card, Group, Stack, Text, Timeline, Title } from '@mantine/core';
...
export function AuctionRouteCard({ route, restrictions }: AuctionRouteCardProps) {
  return (
    <Card withBorder radius="md" padding="md" h="100%">
      <Stack gap="sm">
        ...
        {restrictions.hidePointsAddressAndContacts && (
          <Alert color="gray" icon={<IconEyeOff size={14} />} p="xs">
            Точные адреса и контакты скрыты организатором.
          </Alert>
        )}

        {route.length > 0 ? (
          <Timeline active={route.length} bulletSize={18} lineWidth={2}>
            {route.map((point, index) => (
              ...
              >
                ...
                {point.startDate && (
                  <Text size="xs">Дата: {formatDate(point.startDate)}</Text>
                )}
